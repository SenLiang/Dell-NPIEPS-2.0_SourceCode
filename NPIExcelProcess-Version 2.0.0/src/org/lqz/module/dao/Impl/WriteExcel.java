...
import org.lqz.module.view.ImGeTableJPanel;
...
import static org.lqz.module.view.ImGeTableJPanel.ImportVector;
...
//            sheetdata.add(0, this.titlerow);
    ...
    //创建有数据的工作表，并返回表对象
    public Sheet CreateSheetName(Vector<Vector> sheetdata,String name) throws Exception {
    ...
    public static void SaveToExcel(String exportName, String filepath) {
        ...
        try {
            ...
            if (exportName.equals("AgileBom")) {
                ...
                Vector<Vector> ExportVector= (Vector<Vector>) BOMQueryVector.clone();
                Vector temp = new Vector<String>();
                temp.add("Node");temp.add("Number");temp.add("Level");
                temp.add("Description");temp.add("EMC");
                ExportVector.insertElementAt(temp,0);
                writetoexcel.CreateSheet(ExportVector);
                writetoexcel.SaveToFile();
                ...
                Vector<Vector> ImQueryVector= ImportVector;
                Vector ImportTemp = new Vector<String>();
                ImportTemp.add("Number");ImportTemp.add("Quantity");
                ImQueryVector.insertElementAt(ImportTemp,0);
                writetoexcel.CreateSheetName(ImQueryVector,"Import");
                ...
                temp.add("Description");temp.add("EMC");
                vector.insertElementAt(temp,0);
                writetoexcel.CreateSheetName(vector,"Export");
                writetoexcel.SaveToFile();
