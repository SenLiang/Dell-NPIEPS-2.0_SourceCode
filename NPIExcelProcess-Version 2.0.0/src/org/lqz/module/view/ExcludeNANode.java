...
public class ExcludeNANode extends JFrame {
    ...
    public DefaultMutableTreeNode ExtraverseFolder(int level, int id, int end, int currentLevel) {
        ...
        // System.out.println("level---"+level+"   currentLevel---"+currentLevel);
        if ((getABbyID.getVersion() == null || getABbyID.getVersion().equals("N/A")) && level >= currentLevel)
            return null;//只有make往下继续
        else if (!(getABbyID.getVersion() == null) && getABbyID.getVersion().equals("BUY") && level >= currentLevel) {
            ...
            if (vertorSame.isEmpty()) {//找不出与其相等的值
                ...
                for (Map.Entry<Integer, Level_End> entry : vertorSame.entrySet()) {
                    ...
                    {
                        DefaultMutableTreeNode NewTreeNode = ExtraverseFolder(newLEVELGAP.getLevel(), newID, newLEVELGAP.getEnd(), currentLevel);
                        if (NewTreeNode != null)
