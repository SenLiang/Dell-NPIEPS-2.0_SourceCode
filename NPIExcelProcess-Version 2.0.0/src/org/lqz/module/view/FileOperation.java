...
public class FileOperation {
    ...
    public static Workbook initWorkBook(String mFilePath) throws IOException {
        ...
        try {
            ...
            e.printStackTrace();
            Object[] option = {"OK"};
            JOptionPane.showOptionDialog(null, "Error, please check the file format firstly",
                    "Tips", JOptionPane.YES_OPTION, JOptionPane.QUESTION_MESSAGE, null, option, option[0]);
        }
        ...
        if (JFileChooser.APPROVE_OPTION == fileChooser.showSaveDialog(ParentJFrame)) {
            ...
            if (newFile.renameTo(newFile) || (file.toString().indexOf(ends) == -1)) {
                ...
                if (newFile.exists())//已存在同名文件，弹出提示对话框
                {
                    ...
                    int result = JOptionPane.showOptionDialog(ParentJFrame, "A file with the same name already exists !",
