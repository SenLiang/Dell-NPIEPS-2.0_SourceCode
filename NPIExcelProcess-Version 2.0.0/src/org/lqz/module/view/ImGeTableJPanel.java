...
public class ImGeTableJPanel implements ActionListener {
    ...
    public void QueryChild() {
        StringBuilder sqlBuilder = new StringBuilder(
                "SELECT Quantity, Query_Config.Number, Agile_EMC.Description, New_Version FROM \n" +
                        "                        Query_Config LEFT OUTER JOIN Agile_EMC ON Query_Config.Number=Agile_EMC.Number");
        ...
        if (!list.isEmpty()) {
            for (Object[] object : list) {
                ...
                if (object[2] == null )//Agile_EMC.Description is Null
                {
                    temp.add("Not Found");//level
                    temp.add(object[1]);//number
                    temp.add("Not Found");//desc
                    temp.add("Not Found");//version
    ...
    private static Vector<Integer> qtyVector = new Vector<Integer>();
    ...
    public static Vector<Vector> ImportVector = new Vector<Vector>();
    ...
    public static void MarkID(int NID, int nextNID) throws SQLException {
        ...
        String sql = sqlBuilder.toString();
        List<Object[]> list = select(sql, 7, null);
        if (!list.isEmpty()) {
            for (Object[] object : list) {
...
//        }
        if (vertorSame.isEmpty()) {//找不出与其相等的值
            ...
            for (Map.Entry<Integer, Level_End> entry : vertorSame.entrySet()) {
                ...
                if (AB.getVersion() != null && !AB.getVersion().equals("N/A")) {
                    ...
                    temp.add(Quantity *  Integer.valueOf((String) AB.getQuantity()) );//AB.getId()
                ...
                if (newLEVELGAP.getEnd() != newID && AB.getVersion() != null && !AB.getVersion().equals("BUY") && !AB.getVersion().equals("N/A"))
                    traverseNode(Quantity *  Integer.valueOf((String) AB.getQuantity()), newLEVELGAP.getLevel(), newID, newLEVELGAP.getEnd());
            }
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == tool_import) {
            ...
            if (!validateTableNameExist("Query_Config")) {
                ...
                ImportVector.clear();
                try {
                    ImportVector = QueryCondSelectList();
                } catch (Exception e1) {
                    e1.printStackTrace();
                }
    ...
    //条件查询商品
    public Vector<Vector> QueryCondSelectList() throws Exception {
        Vector<Vector> rows = new Vector<Vector>();
        StringBuilder sqlBuilder = new StringBuilder(
                "select  Number,Quantity from Query_Config;");
        String sql = sqlBuilder.toString();
        List<Object[]> list = select(sql, 2, null);
        if (!list.isEmpty()) {
            for (Object[] object : list) {
                Vector temp = new Vector<String>();
